...
async function generateArticleForClient(client) {
  ...
  try {
    ...
    const metaRaw = metaResponse.content[0].text.replace(/```json|```/g, '').trim();
    const metaMatch = metaRaw.match(/\{[\s\S]*?\}/);
    const meta = metaMatch ? JSON.parse(metaMatch[0]) : { title: keyword, excerpt: businessDesc.substring(0, 155) };
